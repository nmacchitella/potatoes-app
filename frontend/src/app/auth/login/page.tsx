  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        router.push(`/auth/verification-required?email=${encodeURIComponent(formData.email)}`);
      }
    } catch (err: any) {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {isLogin && (
                <div className="flex justify-end">
                  <a href="/auth/forgot-password" className="text-sm text-primary hover:text-primary-hover hover:underline">
                    Forgot Password?
                  </a>
                </div>
              )}
